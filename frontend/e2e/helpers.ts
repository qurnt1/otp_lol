...
export function createState(connected = false, configured = false) {
  ...
  const settings = {
    config_version: "11.0", config_schema_version: 5, auto_accept_enabled: false, auto_pick_enabled: false,
...
export async function mockLocalApi(page: Page, options: { connected?: boolean; configured?: boolean; assignedPosition?: string; historyItems?: unknown[]; rejectPresetActivation?: boolean } = {}) {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (url.pathname === "/api/bootstrap") payload = state;
      ...
      Object.assign(state.settings, route.request().postDataJSON());
      payload = state.settings;
    ...
    else if (url.pathname.startsWith("/api/skins/")) payload = { champion_id: 86, catalog: [{ champion_id: 86, champion_name: "Garen", champion_slug: "Garen", skin_id: 86013, skin_num: 13, skin_name: "God-King Garen", splash_url: demoAsset, tile_url: demoAsset, centered_splash_url: demoAsset, uncentered_splash_url: demoAsset }], owned: { ok: true, owned_skins: [{ skin_id: 86013, skin_num: 13, skin_name: "God-King Garen", preview_url: demoAsset, tile_url: demoAsset, splash_url: demoAsset }], message: "" } };
    else if (url.pathname === "/api/runes") payload = { available: false, pages: [], styles: {} };
    else if (url.pathname === "/api/links/stats") payload = { available: false, site: "opgg", url: null };
