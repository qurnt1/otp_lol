...
export function SettingsPage({ section, onSectionChange }: { section: SettingsSection; onSectionChange: (section: SettingsSection) => void }) {
  ...
  const reset = async () => { setConfirmReset(false); setPending("reset", true); try { const next = await api.resetSettings(); setLocal(next); queryClient.setQueryData(["settings"], next); await Promise.all([queryClient.invalidateQueries({ queryKey: ["stats-link"] }), queryClient.invalidateQueries({ queryKey: ["live-stats-link"] }), queryClient.invalidateQueries({ queryKey: ["bootstrap"] })]); applyTheme(next.theme); setDraft({}); submittedManualValues.current = {}; setFeedback(fr.settings.saved); } catch (error) { setErrors((current) => ({ ...current, reset: error instanceof Error ? error.message : fr.settings.failed })); } finally { setPending("reset", false); } };
  ...
  const detectedRiotId = local.summoner_name_auto_detect && runtime?.connected
    ? runtime.riot_id || ""
    : "";
