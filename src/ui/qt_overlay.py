"""Qt-based in-game stats overlay manager."""
...
class QtStatsOverlayManager:
    """Launch a frameless Qt WebEngine overlay in a dedicated child process."""
    ...
    def show(self, url: str) -> tuple[bool, str]:
        ...
        try:
            process = subprocess.Popen(self._build_launch_command(url), **kwargs)
        except OSError as exc:
            logging.error("Impossible de lancer l'overlay Qt stats: %s", exc)
            self._process = None
            return False, "Impossible de lancer l'overlay Qt stats."
        ...
        return True, "Overlay stats Qt ouvert."
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logging.debug("Impossible d'arreter l'overlay Qt stats: %s", exc)
    ...
    def _build_launch_command(self, url: str) -> list[str]:
        ...
        if getattr(sys, "frozen", False):
            return [sys.executable, "--qt-overlay", *base_args]
        return [sys.executable, "-m", "src.ui.qt_overlay_host", *base_args]
