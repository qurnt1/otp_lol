...
- `#statistics` opens the external profile provider selected by `preferred_stats_site`.
- `#live` opens the configured live-provider profile and uses `preferred_hotkey_site`; a saved identity can open its provider page while League is closed, but cannot supply data for a game that is not currently running.
...
- Provider links and homepages are generated by the local API; embeds are limited to the provider-specific security allowlists, with a browser fallback when embedding is not allowed. Both pages label the account source.
