...
| Skin | `pick_slots[slot].skin_mode`, `skin_automation_enabled` | `test_skin_modes`, matrice API, tests core et E2E Dashboard | Modes `none`, `fixed`, `random` identiques entre l’aperçu et l’éditeur du Dashboard; aucun mode `inherit` ni fallback de mode vers le slot 1 |
