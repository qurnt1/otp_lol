...
Le chemin critique reste local : le WebView démarre FastAPI puis vérifie `/api/network/status` avant d'autoriser `/api/bootstrap`. Si l'origine Data Dragon n'est pas joignable, le frontend affiche l'écran offline avec une image embarquée et bloque le dashboard jusqu'au retour d'Internet. Quand la connexion est disponible, `/api/bootstrap` expose les réglages et aperçus déjà disponibles sans charger les catalogues complets, l'historique, les maîtrises ou les challenges. Au démarrage, OTP LOL lit le snapshot LCU local et le cache Data Dragon depuis le disque. Après connexion du client League, le rafraîchissement du snapshot statique LCU se fait en arrière-plan. Data Dragon reste un fallback de secours.

À l'ouverture de `#statistics` ou `#live`, le frontend choisit le fournisseur dans la page, demande son URL locale, puis délègue l'ouverture à une WebView provider top-level préchargée après l'événement `shown` de la fenêtre principale. Aucune iframe tierce n'est utilisée. Le bouton Actualiser recharge la WebView existante; le navigateur système est le fallback volontaire. Les diagnostics et leurs tests d'endpoints ne s'exécutent que depuis la page Diagnostics; aucun catalogue items complet ni challenge n'est chargé au démarrage.
