...
| Auto-Summs / Auto-Runes | Applique les réglages du slot seulement si l'option est active | Preset slot + runtime; le réglage désactivé bloque aussi les runes | Couvert | `tests/test_core_champ_select.py`, `tests/test_lcu_runtime.py` | Conserver le gate au lancement et avant l'écriture LCU |
...
| Instance unique / format des réglages | Lockfile et paramètres TOML | Launcher WebView; schéma 6 strict, création au premier lancement, aucune migration d'ancien schéma | Couvert | `tests/test_config.py`, `tests/test_api.py` | Schéma absent/ancien remplacé par défaut sans `.bak`; schéma futur ou fichier invalide sauvegardé avant remplacement |
